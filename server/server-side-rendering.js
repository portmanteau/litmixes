...
onPageLoad(sink => {
  ...
  if (slugArray.length > 0) {
    ...
    const backgroundUrl =
      (mix && mix.backgroundUrl) || '/space-sun-solar-flare-animation-8.gif';

    sink.appendToHead(`<title>${slug}</title>`);
    sink.appendToHead(`<meta property="og:type" content="article" />`);
    sink.appendToHead(
      `<meta property="og:url" content="http://litmix.es/${slug}" />`,
    );
    sink.appendToHead(
    ...
    sink.appendToHead(`<meta property="og:url" content="http://litmix.es/" />`);
    sink.appendToHead(
      `<meta property="og:image" content="/space-sun-solar-flare-animation-8.gif" />`,
    );
    sink.appendToHead(`<meta property="og:title" content="LitMixes" />`);
    sink.appendToHead(
      `<link rel="icon" href="/space-sun-solar-flare-animation-8.gif"/>`,
    );
  }
});
